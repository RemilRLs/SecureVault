...
int manage_user_session(char *authenticated_username) {
    ...
    if(verify_master_password(username, passwd)){
...
int verify_master_password(const char* username, const char *passwd){
    char file_path[FILE_PATH_SIZE];
    generate_master_password_file_path(username, file_path);
